...
const calculateTotalQuantity = computed(() =>
  selectedProducts.reduce(
    (total, product) =>
      total + (product.inventoryQuantity === 0 
        ? 0 
        : product.shoppingQuantity),
    0
  )
);

// 计算税
const calculateTax = (sumPrice) =>
  sumPrice <= 40 
    ? 0 
    : parseFloat((sumPrice * 0.05).toFixed(1));

// 限制购买数量
const limitQuantity = (product) => {
  if (product.shoppingQuantity > 30) product.shoppingQuantity = 30;
  
  if (!(product.shoppingQuantity)) product.shoppingQuantity = 1;
};

// 计算总结算价格
const calculateSettlementPrice = (sumPrice, tax, freight) =>
  parseFloat(
    (sumPrice <= 40 
      ? sumPrice + freight 
      : sumPrice + tax + freight
    ).toFixed(1)
  );
...
// computed 属性计算动态数据
const lumpSum = computed(() => calculateLumpSum(selectedProducts));
const taxAmount = computed(() => calculateTax(lumpSum.value));
const settlementPrice = computed(() =>
  calculateSettlementPrice(
    lumpSum.value,
    taxAmount.value,
    calculateTotalQuantity.value
  )
);
</script>
